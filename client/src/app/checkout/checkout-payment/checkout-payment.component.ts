...
export class CheckoutPaymentComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.stripe = Stripe('pk_test_cWTE4dbbGuSZEZHg2gDv0uRP00U3TvfUU0');
  ...
  private async confirmPaymentWithStripe(basket: IBasket): Promise<any> {
    return this.stripe.confirmCardPayment(basket.clientSecret, {
      payment_method: {
        ...
        billing_details: {
          name: this.checkoutFrom.get('paymentForm').get('nameOnCard').value
        }
      }
    });
  }
  ...
  async submitOrder(): Promise<any> {
    ...
    try {
      ...
      const {error, paymentIntent} = await this.confirmPaymentWithStripe(basket);
      ...
      this.loading = false;
      this.toastr.warning('An error occurred while processing payment');
    }
  }
}
